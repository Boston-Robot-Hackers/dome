...
// Run each motor at it's "maximum" in each direction, which is + or - 255. And then report on the RPM achieved in each direction. 
// Finally compute an averqge resulting speed  linear and rotational for each motor.
...
void loop() {
    ...
    Serial.printf("MOTOR%d %s RPM 1: %4.1f 2: %4.1f 3: %4.1f 4: %4.1f\n",
	   current_motor + 1, direction ? "REV" : "FWD",
	   current_rpm1, current_rpm2, current_rpm3, current_rpm4);
    syslog(LOG_INFO, "MOTOR%d %s RPM 1: %8.1f 2: %8.1f 3: %8.1f 4: %8.1f\n",
	   current_motor + 1, direction ? "REV" : "FWD",
	   current_rpm1, current_rpm2, current_rpm3, current_rpm4);
    tk++;
    runWifis();
